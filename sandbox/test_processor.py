...
class ReviewProcessor:
    def __init__(self, review_filename):
        ...
        # self.restaurant_ids = set([iD.strip() for iD in open('../Data/Phoenix_restaurant_ids.txt', 'rb')])
...
if __name__ == "__main__":
    review_fileName = 'Data/Arts.votes'

    if not path.exists('Data/data.csv'):

        with open(review_fileName, 'rb') as review_file:
            output_file = open('Data/data.csv', 'wb')
            for line in review_file.readlines():
                ...
                line = ','.join([user_id, business_id, stars, timestamp, length, text])
